...
### 变更
- 优化 | EaseTrojan 算法现在以**更数学的方法尝试解密资源中心二次加密的地图**
> [!TIP]
> 其思想为“用已知量求解未知密钥流”的数学思想，利用 `CURRENT` 文件的固定结构（小端版本号 + `MANIFEST` 文件名）作为 `crib`，计算出密钥流，再检测其循环周期并解密，最后用格式验证器确保正确性，早期版本的密钥法会予以保留**进行一次解密，以便炸弹机开始基于一次解密的密文开始二次数学解密**

- 优化 | 进度条会展示 `实际耗时 / ETA`，这对于性能敏感的用户可能会起到一点（微不足道的）性能基准参考
> [!NOTE]
> 最低可被记录的单位是微秒（μs）。如果你**一不小心或故意进入了比微秒更小的时间单位，请告诉我们，以便我们继续进行更精确的秒单位计算与展示**<br>
> 顺带一提，这是我提出的挑战（笑）
